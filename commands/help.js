...
module.exports.run = async (bot, message, args) => {
    ...
    if(!helpArgs[0]) {
        var embed = new Discord.MessageEmbed()
            ...
            .setDescription('```Serverinfo(sends severinfo) | userinfo(sends userinfo) | Ping(pings the bot)| uptime(sends how log the bot has been up)| meme(send a random meme) | random(send random stuff from random redits) |sucks(sends a random image or video from /r/wellthatsucks| hot(sends random gif from hotgirls subredit) | gif(sends random gif from /r/gif subredit) |```')
